...
def train_model(config):
    ...
    Path(config['model_folder']).mkdir(exist_ok=True, parents=True)  # Create model folder if it does not exist    
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # run tensors through transfomrer model
            encoder_output = model.encode(encoder_input, encoder_mask)
            decoder_output = model.decode(encoder_output, encoder_mask, decoder_input, decoder_mask)
            projection_output = model.project(decoder_output)

            # compare to label 
            label = batch['decoder_input'].to(device)
            ...
            global_step += 1 # mostly for tensorboard to trafck the loss. 

        # save model per epcoh  
